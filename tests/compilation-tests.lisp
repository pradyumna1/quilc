  ...
  (labels ((random-rotation (name &rest qubits)
    ...
    (let ((template-list '(("I")
      ...
      (dolist (template template-list)
        (format fiasco::*test-run-standard-output* "    Trying test template ~19a" template)
          ...
          (let* ((ref-mat (cl-quil::make-matrix-from-quil random-quil))
                 ...
                 (big-gates (mapcar (alexandria:compose
                                    ...
                                    (remove-if (lambda (i) (= 1 (length (application-arguments i))))
                                               processed-quil))))
            (format fiasco::*test-run-standard-output* ", got ~19a back.~%" big-gates)
            (is (cl-quil::matrix-equality ref-mat
                                          (cl-quil::scale-out-matrix-phases mat ref-mat)))
            (when (> (length big-gates) (length template))
              (format fiasco::*test-run-standard-output* "    WARNING: deeper answer than expected~%"))))))))
...
(deftest test-cnot-rewiring-in-cnot-architecture ()
  ...
  (format fiasco::*test-run-standard-output* "[Test output: ~%")
  (finish-output)
  ...
  (let ((fiasco:*print-test-run-progress* nil)
    ...
    (dotimes (i 16)
      ...
      (dotimes (j 16)
        (if (/= i j)
            (progn (format fiasco::*test-run-standard-output* " ~X~X" i j)
